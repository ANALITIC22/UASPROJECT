/**
 * planeamiento.js — Módulo de Planeamiento de Misión
 * ===================================================
 * Muestra registros de PLANEAMIENTO_OPS.xls (GMB-F08).
 */

const PlaneamientoModule = (() => {
  ...
  function render() {
    ...
    let records = (data.planeamiento || []).slice();
    ...
    const tbody = document.getElementById('tbl-plan');
    if (!tbody) return;

    if (records.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text2);padding:32px">
        ${_filter ? `Sin registros para ${Formatter.shortName(_filter)}` : 'Sin registros de planeamiento'}
      </td></tr>`;
      _setText('pla-total', 0);
      return;
    }
    ...
    tbody.innerHTML = records.map((r, i) => {
      ...
      return `<tr class="animate-fade-in">
        ...
        <td style="text-align:center">${Formatter.badge(r.formulario || 'GMB-F08', 'purple')}</td>
      </tr>`;
    }).join('');

    _setText('pla-total', records.length);
    _bindSortHeaders();
  }

  function _bindSortHeaders() {
    document.querySelectorAll('[data-sort-pla]').forEach(th => {
      th.onclick = () => {
        const col = th.dataset.sortPla;
