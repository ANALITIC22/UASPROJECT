 ...
 * ================================================
 * Módulo principal de persistencia para UASOPS v8.
...
// ── Configuración del proyecto (desde config centralizada) ──────
...
const SupabaseDB = (() => {
  ...
  /**
   * Elimina TODOS los registros de todas las tablas (mantiene las tablas vacías).
   */
  async function deleteAllCollections() {
    let totalDeleted = 0;
    const ALL_TABLES = [
      ...DATA_KEYS.map(k => TABLES[k]),
      PILOTS_TABLE,
      CLIENTS_TABLE,
      '_ping',
    ];
    ...
    for (const table of ALL_TABLES) {
      try {
        ...
        if (countErr) {
          console.warn(`[Supabase] No se pudo contar "${table}":`, countErr.message);
          continue;
        }

        if (!count || count === 0) {
          console.info(`[Supabase] "${table}" ya está vacía`);
          continue;
        }

        // 2. Eliminar todos los registros (en lotes de 1000 si hay muchos)
        let remaining = count;
        while (remaining > 0) {
          const { data, error: delErr } = await _sb
            .from(table)
            .delete()
            .select('id');

          if (delErr) {
            console.error(`[Supabase] Error eliminando "${table}":`, delErr.message);
            break;
          }
          ...
          // Si no se eliminó nada, salimos del loop
          if (deleted === 0) break;
        }

        console.info(`[Supabase] "${table}": ${count} registros eliminados`);
      } catch (err) {
        console.error(`[Supabase] Excepción en "${table}":`, err.message);
      }
    }
